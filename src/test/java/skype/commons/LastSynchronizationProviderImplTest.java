...
public class LastSynchronizationProviderImplTest {
	...
	@Test
	public void getLastSynchWhenNoPlaceHolderExists()
	{
		SkypeStorage storage = new SkypeStorageMock("mock-1");
		LastSynchronizationProviderImpl subject = createSubject(storage);
		Date lastSynch = subject.getLastSynch();
		Calendar instance = Calendar.getInstance();
		instance.setTime(lastSynch);
		Assert.assertEquals(1900, instance.get(Calendar.YEAR));
	}
	
	private void checkStorageTime(SkypeStorage firstStorage,
			int lastSyncYear) throws IOException {
		LastSynchronizationProviderImpl subject = createSubject(firstStorage);
	...
	private LastSynchronizationProviderImpl createSubject(
			SkypeStorage firstStorage) {
